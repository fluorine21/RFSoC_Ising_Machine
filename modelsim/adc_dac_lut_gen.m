...
function [lut_dac, lut_adc] = gen_nl_luts(V_in, I_out)
    ...
    %Find the position of the closest 0
    zero_pos = find_zp(V_in, I_out, I_min_pos);
    ...
    v_neg = [];
    i_neg = [];
    if(zero_pos > I_min_pos)%If we were going in the positive direction
        %Flip here so we always count from zero on up
        v_neg = flip(V_in(I_min_pos : zero_pos));
        i_neg = flip(I_out(I_min_pos : zero_pos));
    else
        v_neg = V_in(zero_pos : I_min_pos);
        i_neg = I_out(zero_pos : I_min_pos);
    end
    ...
    zero_pos = find_zp(V_in, I_out, I_max_pos);
    
    v_pos = [];
    i_pos = [];
    if(zero_pos > I_max_pos)%If we were going in the positive direction
        %Flip here so we always count from zero on up
        v_pos = flip(V_in(I_max_pos : zero_pos));
        i_pos = flip(I_out(I_max_pos : zero_pos));
    else
        v_pos = V_in(zero_pos : I_max_pos);
        i_pos = I_out(zero_pos : I_max_pos);
    end
...
function zp = find_zp(v_in, i_in, peak_pos)
